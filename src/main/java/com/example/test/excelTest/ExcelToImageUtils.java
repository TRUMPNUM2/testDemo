package com.example.test.excelTest;
...
public class ExcelToImageUtils {
    private static Double ratis = 1.00;
    private static Double hRatis = 1.00;
    private static Double rBRatis = 1.00;
    private static Double hBRatis = 1.00;
    ...
    public static void main(String[] args) throws Exception {
        ...
        /*WorkbookFactory.create(new File("C:/Users/admin/Downloads/缴退费信息 (27).xlsx"));*/
        XSSFWorkbook wb = new XSSFWorkbook(new FileInputStream("C:/Users/admin/Downloads/缴退费信息 (27).xlsx"));
        ...
        for (int i = 0; i < rowSum; i++) {
            colSum = colSum > sheet.getRow(0).getPhysicalNumberOfCells() ? colSum : sheet.getRow(0).getPhysicalNumberOfCells();
        }
      ...
      /*  if (fromIndex[1] > colSum || fromIndex[1] > toIndex[1] || toIndex[1] > colSum) {
        ...
        for (int i = 0; i < rowSize; i++) {
            ...
            for (int j = 0; j < colSize; j++) {
                ...
                colPixPos[j + 1] = (int) (widthPix * ratis + colPixPos[j]);
            ...
            rowPixPos[i + 1] = (int) (heightPoint * hRatis) + rowPixPos[i];
        ...
        for (int i = 0; i < rowSize; i++) {
            for (int j = 0; j < colSize; j++) {
                ...
                grid.setWidth((int) ((colPixPos[j + 1] - colPixPos[j]) * rBRatis));
                grid.setHeight((int) ((rowPixPos[i + 1] - rowPixPos[i]) * hBRatis));
                ...
                grid.setFont(font);
                HorizontalAlignment alignmentEnum = cs.getAlignmentEnum();
                grid.setHorizontalAlignmentCode(alignmentEnum.getCode());
        ...
        for (Grid g : grids) {
            ...
            g2d.setFont(font);
            /*处理左对齐，右对齐，居中对其*/
            if (g.getHorizontalAlignmentCode() == HorizontalAlignment.RIGHT.getCode()) {
                g2d.drawString(g.getText(),
                        g.getX() + (g.getWidth() - strWidth),
                        g.getY() + (g.getHeight() - font.getSize()) / 2 + font.getSize());
            } else if (g.getHorizontalAlignmentCode() == HorizontalAlignment.CENTER.getCode()) {
                g2d.drawString(g.getText(),
